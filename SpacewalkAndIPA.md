...
New with Spacewalk 2.1, it is possible to set up the Spacewalk server WebUI authentication to use external identity providers and management solutions, for example [http://www.freeipa.org/ FreeIPA] or [https://access.redhat.com/site/documentation/en-US/Red_Hat_Enterprise_Linux/6/html-single/Identity_Management_Guide/ Identity Management]. The new feature extends the original per-user PAM authentication, and includes support for:
...
spacewalk-setup-2.3.1-1 or newer, you may use [#automatic-setup spacewalk-setup-ipa-authentication script] to 
configure pam, sssd, apache and tomcat for you. Otherwise, you need to follow the [#manual-setup manual steps].

== Automatic setup using the spacewalk-setup-ipa-authentication script == #automatic-setup
...
Continue with [#hbac Host based access control prerequisites]
...
== Host based access control prerequisites == #hbac
