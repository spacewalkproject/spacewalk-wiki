...
== Requirements ==
  ...
  * ''Pretty sure we already have this, see current hardware info example below. cat /proc/cpuinfo on a multi-cpu multi-core system looks like we'll have enough info to work with.'' - dgoodwin
...
* Cons
  * Ruby not Python ( but has Python APIs - [The kernel is C right? ;) ]
