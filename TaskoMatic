...
The Java port of Taskomatic is based on the [http://quartz-scheduler.org/ Quartz] scheduler library. The daemon part of Taskomatic uses the [http://wrapper.tanukisoftware.org/doc/english/introduction.html Tanuki Service Wrapper] library.
...
To check whether the engine is running and what is the last time each bunch was ran and its state, check ''Admin -> Task Engine Status'' and note "Scheduling Service: ON" and status and execution date of individual bunches.
...
Navigate to ''Admin -> Task Schedules''. There is a list of defined task schedules with given frequencies as [http://www.quartz-scheduler.org/documentation/quartz-1.x/tutorials/crontrigger Quartz strings]. After navigating to selected schedule, you can edit Quartz string manually or select values from form.
...
See also: [wiki:LogDriverSetup LogDriverSetup]
