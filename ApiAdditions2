...
||  errata.setDetails([[BR]]string sessionKey, [[BR]]struct(errata details)[[[BR]]string synopsis, [[BR]]string advisoryName, [[BR]]int advisoryRelease, [[BR]]string advisoryType, [[BR]]string product, [[BR]]string topic, [[BR]]string description, [[BR]]string solution, [[BR]]   string references, [[BR]]string notes, [[BR]]array[struct(bug)[int id, string summary]], [[BR]]array[string keyword]) || 0.4 || 3 || bbuckingham || done || * Set errata details.  All arguments are optional and will only be modified if included.[[BR]] * Returns 1 on success, exception thrown otherwise. ||
||  errata.addPackages(string sessionKey, string advisoryName, array[int packageId]) || 0.4 || 2 || bbuckingham || done || * Adds a set of packages to an errata.  Invoking this API will not impact packages already associated with the errata.[[BR]] * Returns the number of packages added on success, exception thrown otherwise.  ||
||  errata.removePackages(string sessionKey, string advisoryName, array[int packageId]) || 0.4 || 2 || bbuckingham || done || * Remove a set of packages from an errata.[[BR]] * Returns the number of packages removed on success, exception thrown otherwise.  ||
||  ----   ||  ||  ||  ||  ||  ||
||  '''Namespace: kickstart''' ||  ||  ||  ||  ||  ||
...
||  kickstart.profile.system.addKeys(string sessionKey, string kslabel, array[string description]) || 0.3 || 4 || jdob || done ||* Add GPG and SSL keys to the kickstart profile.[[BR]]'''* Renamed from associateKeys to addKeys in 0.4 - bbuckingham''' ||
||  kickstart.profile.system.removeKeys(string sessionKey, string kslabel, array[string description]) || 0.4 || 4 || bbuckingham || done || Remove GPG and SSL keys from the kickstart profile.||
||  ----   ||  ||  ||  ||  ||  ||
...
||  kickstart.filepreservation.create(string sessionKey, string name, array[string fileName]) || 0.4 || 3 || bbuckingham || done || Create a file preservation list. ||
...
||  schedule.cancelActions(string sessionKey, array[int actionId]) || 0.4 || 3 || bbuckingham || done ||  ||
||  ----   ||  ||  ||  ||  ||  ||
...
||  system.schedulePackageRemove(string sessionKey, int serverId, array[int packageId], dateTime.iso860 earliestOccurrence) || 0.4 || 4 || bbuckingham || done || * Schedule a package remove for a system.[[BR]]* Returns the id of the scheduled action (int actionId), exception thrown otherwise. ||
...
||  system.setChildChannels(string sessionKey, int serverId, array[int channelId OR string channelLabel]) || 0.5 || 4 || bbuckingham || done || * Set a system's child channels.  This is an existing API that takes an array[int channelId] that has been enhanced to take an array[string channelLabel] instead.  The intent is to deprecate the usage of array[int channelId]. ||
...
||  system.deleteCustomValues(string sessionKey, int serverId, array[string customInfoLabel]) || 0.5 || 4 || bbuckingham || done || * Delete one or more custom values from a system.  ||
