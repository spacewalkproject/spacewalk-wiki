...
To support group operations for RHEL 5 clients fast, we have hardcoded paths to comps files in the kickstart trees in the server code, done in [https://bugzilla.redhat.com/show_bug.cgi?id=247156 bugzilla 247156]. However, that means that the kickstart trees need to be synced, and the paths always point to the 5.0 GA files, so for example RHEL 5.5 still gets 5.0 comps.xml file.

In [https://bugzilla.redhat.com/show_bug.cgi?id=585233 bugzilla 585233] we aim at supporting the comps syncing properly, both from RHN hosted to RHN Satellite, to channel dumps, and across Satellites.

== RHN hosted support ==

RHN hosted has added the support for exposing the comps data for channels in [https://bugzilla.redhat.com/show_bug.cgi?id=249141 bugzilla 249141]. It has two parts:
...
For Satellite 5.3.*, errata [http://rhn.redhat.com/errata/RHBA-2010-0443.html RHBA-2010:0443-1] includes the changes.