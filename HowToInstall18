...
These are installation instructions for new installations of Spacewalk 1.8. If you are upgrading from older versions, see HowToUpgrade.

Spacewalk 1.7 installation instructions are available at [wiki:HowToInstall17].
...
If you prefer to setup the PostgreSQL manually, you have a choice to install it on the same machine as Spacewalk or different machine. Use [wiki:PostgreSQLServerSetup] as a guide to get the server installed and setup. Namely, you need a database and a user, the user should be a superuser and the database should have the plpgsql language created.
...
On system with {{{firewalld}}} use {{{firewall-cmd --add-service=http --add-service=http}}}. 
