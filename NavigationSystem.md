...
[[Image(wiki:NavigationSystem:sitenav.png)]]
...
=== Behind the scenes ===
I'm sure you want to know all of the magic that goes on behind the scenes. The driving code is the [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/NavDigester.java NavDigester] class. It uses {{{commons-digester}}} to parse the navigation files and create the appropriate !NavTree.
...
[http://commons.apache.org/digester/ Digester] offers an easy way to define how to build objects from an XML file. Given a sitenav.xml file, it will create a [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/NavTree.java NavTree] object which contains a {{{NavNode}}} for each of the rhn-tab tags found. Once the {{{NavTree}}} is created, we then pass it to the [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/NavTreeIndex.java NavTreeIndex] to index the tree by urls so that it makes it easier to do matching while rendering the UI. This allows the appropriate tab to be highlighted as well as the page title to be set properly.
...
==== Rendering ====
Navi has a [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/RenderEngine.java RenderEngine] that is used to display the menus of varying levels. The {{{RenderEngine}}} takes in a {{{NavTreeIndex}}} and a URL. As part of the rendering process we had a need to render the menus in different ways and also turn off some based on [#ACLs ACLs].

To accomplish the displaying of the menus, Navi has a [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/Renderable.java Renderable] abstract class used to implement different {{{Renderers}}}. Out of the box Navi has five (5) renderers:
|| !DialogNavRenderer || [[Image(wiki:NavigationSystem:dialognav.png)]]
|| !SidenavRenderer ||  [[Image(wiki:NavigationSystem:leftnav.png)]]
|| !TextRenderer || [[Image(wiki:NavigationSystem:textnav.png)]]
|| !TitleRenderer || [[Image(wiki:NavigationSystem:title.png)]] [[Image(wiki:NavigationSystem:title-html.png)]]
|| !TopnavRenderer || [[Image(wiki:NavigationSystem:topnav.png)]]

Lastly, the [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/taglibs/NavMenuTag.java NavMenuTag] is a jsp tag used to actually render the menu to the screen. The !NavMenuTag determines the url from the Request object and calls into the !NavTreeIndex to determine what to output.
...
==== Guarding ====
Guarding is the ability to enable/disable the rendering of a given node based on any boolean expression as defined by the [#ACLs ACL system], be it a configuration entry, dynamic application data, or even assigned user roles. Navi takes the same approach to guarding as it did with the rendering, there is a [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/RenderGuard.java RenderGuard] interface which simply returns {{{true}} or {{{false}}} on whether the {{{NavNode}}} can be rendered.

Navi contains two (2) guards: [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/AclGuard.java AclGuard], which returns the result of evaluating the [#ACLs ACL], and [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/DepthGuard.java DepthGuard], which evaluates the depth of the menu. 

There is also a need to group !RenderGuards, hence the creation of [https://github.com/spacewalkproject/spacewalk/blob/master/java/code/src/com/redhat/rhn/frontend/nav/RenderGuardComposite.java RenderGuardComposite]. !RenderGuardComposite takes a list of !RenderGuards and evaluates them in order as an '''and''' operation. First !RenderGuard to fail causes the entire operation to return {{{false}}}.