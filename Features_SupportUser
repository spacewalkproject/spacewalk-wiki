...
[wiki:SupportUserRequirementsDiscussion Requirements Discussion]
 ...
 1. Users in the support user role have access to new search functionality for their needs.
   * Support user searches must be able to search across multiple organizations (all must be trusted by the home org of the user).
    ...
    * The user must be able to select a subset of ''their'' allowed organizations at the time of search.
...
''TODO: Revisit this after Requirements/Implementation discussions.''
...
|| '''Description''' || '''Estimate''' || '''Confidence''' ||
|| Implement page || 8h || 3 ||
'''Total: 1d''' ''This one is a bit fuzzy since we haven't fully flushed out what capabilities it will provide.''
...
'''Total: 1d'''
''This investigation may yield small to medium sized tasks relating to existing pages (i.e. what to do about SDC pages for systems in non-home orgs.''

Adding up the estimated totals from above, the total time is '''17 days''' ''(Note: this value is still very much a work in progress, this is just a first pass)''
